...
# продвинутая авторизация с заменой формы редактирования
...
def login(request):
    ...
    title = "Логинимся"
    context = {"page_title": title, "form": form}
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if send_verify_mail(user):
                ...
                return HttpResponseRedirect(
                    reverse("auth:login")
                )  # после регистрации переадресация
            else:
            ...
            context[
                "message"
            ] = f'пользователь {request.POST.get("email")} уже существует'
...
def userlist(request):
    if not request.user.is_superuser:
        ...
        )  # уходим на главную если мы не админ
